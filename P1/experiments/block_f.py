...
def _worker(args):
    ...
    agent = ImprovedIPPO(cfg.N_total, cfg=cfg, lr=3e-4, device=device) if algo=="Improved_IPPO" else IPPO(cfg.N_total, cfg=cfg, lr=3e-4, device=device)
    rows=[]
...
def run_block_f(log_dir='P1/logs', n_seeds=N_SEEDS, n_episodes=N_EPISODES, n_windows=N_WINDOWS, n_workers=None, device="cpu"):
